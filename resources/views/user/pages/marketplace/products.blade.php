...
<div class="mp-grid-wrap mt-4">
    <div class="mp-grid-header">
        <h2>Browse Digital Products</h2>
        <p>
            Discover ready-made digital products with instant download after purchase -- templates, scripts, ebooks, graphics and more.
        </p>
    </div>
    ...
    <div class="mb-4">
    <a href="{{ route('user.marketplace.products') }}" class="wu-category-pill {{ !isset($category) ? 'active' : '' }}">All</a>

    @foreach($categories as $cat)
        <a href="{{ route('user.marketplace.products.category', $cat->slug) }}" class="wu-category-pill {{ (isset($category) && $category->id == $cat->id) ? 'active' : '' }}">
            {{ $cat->name }}
        </a>
    @endforeach
</div>
    <div class="row">
        ...
        @empty
            <div class="col-12">
                <div class="alert alert-info">No digital products available right now.</div>
            </div>
        @endforelse
    </div>
