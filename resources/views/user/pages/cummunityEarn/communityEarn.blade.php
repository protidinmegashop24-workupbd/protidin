            ...
            <form action="{{route('user.communityPostStore')}}" method="POST" class="create-post-container" enctype="multipart/form-data" id="main-post-form">
                ...
                <div id="editor-state">
                    ...
                    <textarea
                        ...
                        id="post-input"
                        class="post-textarea"
                        placeholder="Write your post here... (আর্টিকেল/Q&amp;A এর জন্য কমপক্ষে ২০ শব্দ আবশ্যক)"
            ...
            @foreach($posts as $post)
                ...
                <div class="post-card" id="post-{{$post->id}}">
                    ...
                    @php
                        ...
                        $teaser = (!$isProductPost) ? community_teaser($post->postContent, 15) : null;
                    @endphp
                    <div class="post-body">
                        <div class="post-main-content">
                            ...
                            @if($post->fetchUrl)
                                ...
                                @elseif(!$hasOwnMedia)
                                    {{-- The link preview box is a fallback for when there's no
                                         own photo -- the link itself is already clickable inline
                                         in the post text above (auto-linkified when pasted), so
                                         once the user uploads their own photo this whole box
                                         would just be a redundant repeat of that same link. --}}
...
<script>
    ...
    function handleInput(el) {
        ...
        if (typeof currentPostType !== 'undefined' && currentPostType === 'article') {
            counter.innerText = words + ' / 20';
            counter.style.color = words >= 20 ? '#198754' : '#dc3545';
            if (suffix) suffix.innerText = ' শব্দ (কমপক্ষে ২০ আবশ্যক)';
        } else {
    ...
    function setPostType(type, btn) {
        ...
        if (postInput) {
            postInput.placeholder = isProduct
                ...
                : 'Write your post here... (আর্টিকেল/Q&A এর জন্য কমপক্ষে ২০ শব্দ আবশ্যক)';
    ...
    document.getElementById('main-post-form').addEventListener('submit', async function(e) {
        ...
        if (currentPostType === 'article') {
            ...
            const words = text ? text.split(/\s+/).length : 0;
            if (words < 20) {
                toastr.error('বিবরণ কমপক্ষে ২০ শব্দ হতে হবে (বর্তমানে ' + words + ' শব্দ)।');
                return;
            }
        }
