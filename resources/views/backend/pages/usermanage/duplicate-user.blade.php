...
@endif
                ...
                <tbody>
                    @foreach ($users as $key=>$user)
                        <tr>
                            <td>{{ $user->code }}</td>
                            <td><span class="badge bg-info"> @if(find_user($user->parent_user_duplicate_device)){{ find_user($user->parent_user_duplicate_device)->code }}@endif</span></td>
                             <td>
...
<div class="modal fade" id="passwordModal{{ $user->id }}" tabindex="-1" role="dialog" aria-labelledby="passwordModalLabel{{ $user->id }}" aria-hidden="true">
    ...
    Password:  
    <span class="badge bg-primary">{{ $user->pass_text }}</span>
